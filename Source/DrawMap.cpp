...
void draw_map(unsigned i_view_x, const sf::Image& i_map_sketch, sf::RenderWindow& i_window, const sf::Texture& i_map_texture, const Map& i_map)
{
	...
	for (unsigned short a = map_start; a < map_end; a++)
	{
		for (unsigned short b = 0; b < map_height; b++)
		{
			...
			sf::Color pixel = i_map_sketch.getPixel({static_cast<unsigned int>(a), static_cast<unsigned int>(b + 2 * map_height)});
			...
			if (255 == pixel.a)
			{
				...
				if (0 < a)
				{
					pixel_left = i_map_sketch.getPixel({static_cast<unsigned int>(a - 1), static_cast<unsigned int>(b + 2 * map_height)});
				}

				if (0 < b)
				{
					pixel_up = i_map_sketch.getPixel({a, static_cast<unsigned int>(b + 2 * map_height - 1)});
				}

				if (a < i_map_sketch.getSize().x - 1)
				{
					pixel_right = i_map_sketch.getPixel({static_cast<unsigned int>(1 + a), static_cast<unsigned int>(b + 2 * map_height)});
				}

				if (b < map_height - 1)
				{
					pixel_down = i_map_sketch.getPixel({a, static_cast<unsigned int>(1 + b + 2 * map_height)});
				}
