	...
	if (0 == dead)
	{
		if (-CELL_SIZE < y && x >= static_cast<int>(i_view_x) - CELL_SIZE - ENTITY_UPDATE_AREA && x < ENTITY_UPDATE_AREA + SCREEN_WIDTH + i_view_x && y < SCREEN_HEIGHT)
		{
			...
			else
			{
				...
				collision = i_map_manager.map_collision({Cell::ActivatedQuestionBlock, Cell::Brick, Cell::Pipe, Cell::QuestionBlock, Cell::Wall}, sf::FloatRect({x, vertical_speed + y}, {static_cast<float>(CELL_SIZE), static_cast<float>(CELL_SIZE)}));
				...
				horizontal_hit_box = sf::FloatRect({MUSHROOM_SPEED * horizontal_direction + x, y}, {static_cast<float>(CELL_SIZE), static_cast<float>(CELL_SIZE)});
